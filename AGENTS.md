...
## Editorial style (letter and drafts)

- Do **not** use em dashes (`—`) or en dashes (`–`) in prose for the letter, `tasks/`, or borradores. Prefer commas, colons, parentheses, or a new sentence.
- Keep the public, dialogue-oriented register of the open letter; criticize decisions and processes, not persons by name when the issue is institutional.
...
| `/grillar-punto` | `grillar-punto` | Relentless one-question-at-a-time interview on a backlog point |
